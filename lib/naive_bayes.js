 ...
 *   , useWordFrequency: true
 * })
 *
 * Takes an (optional) options object containing:
 *   - `categories` => array of categories to classify documents with.
 *   
 *   - `useWordFrequency` => boolean:
 *   
 *      if true: looks at word frequencies in document (default)
 *      
 *      if false: only looks at unique words in the document 
 *                (this has better results for sentiment analysis)
 * 
...
function Naivebayes (options) {
  ...
  if (!options.categories)
    ...
    throw new Error('NaiveBayes must receive an array as its first argument')
